...
public class BinaryNode<AnyType> {
	...
	public int lowness(BinaryNode<AnyType> t) {
		if (t == null)
			return -1;
		return 1 + Math.min(lowness(t.right), lowness(t.left));
	}
	...
	private boolean balanced1(BinaryNode<AnyType> t) {
		if (t == null)
			return true;
        return (Math.abs(height(t.left) - height(t.right)) <= 1) && balanced1(t.right) && balanced1(t.left);
	}
	...
	public boolean balanced2() {
		return Math.abs(balanced2(this)) <= 1;
	}
	
	private int balanced2(BinaryNode<AnyType> t) {
		if ( t == null )
			return -1;
		return 1 + Math.max(balanced2(t.left), balanced2(t.right));
	}
