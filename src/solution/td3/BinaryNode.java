...
public class BinaryNode<AnyType> {
	...
	public int heightWithColor(){
		return heightWithColor(this, this.data, -1);
	}

	//ne pas changer les identificateurs utilisés ci-dessous
	private int heightWithColor(BinaryNode<AnyType> t1, AnyType x, int lg){
		if (t1 == null)
			return lg;
		if (t1.data == x)
			return Math.max(heightWithColor(this.left, x, lg + 1), heightWithColor(this.right, x, lg + 1));
		return lg;
	}

	public boolean parfait1() {
		return parfait1(this);
	}

	//ne pas changer les identificateurs utilisés ci-dessous
	private boolean parfait1(BinaryNode<AnyType> t) {
		if (t == null)
			return true;
		return t.left.size() == t.right.size() && parfait1(this.left) && parfait1(this.right);
	}

	public boolean parfait() {
		return parfait(this.left) == parfait(this.right);
	}

	//ne pas changer les identificateurs utilisés ci-dessous
	private int parfait(BinaryNode<AnyType> t) {
		if (t == null)
			return 1;
		if (t.left != null && t.right != null && ((t.left.left != null && t.left.right != null && t.right.right != null && t.right.left != null) ||((t.left.left == null && t.left.right == null && t.right.right == null && t.right.left == null))) || t.left == null && t.right == null) {
			return 1 + parfait(this.left) + parfait(this.right);
		}
		return 0;
	}
