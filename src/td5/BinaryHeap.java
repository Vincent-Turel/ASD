...
public class BinaryHeap<AnyType extends Comparable<? super AnyType>> {
	...
	private Comparator<AnyType> c = (e1, e2) -> e1.compareTo(e2);
	...
	private void percolateDown(int n) {
		if (right(n) < size) {
			int maxChild = A[left(n)].compareTo(A[right(n)]) > 0 ? left(n) : right(n);
			if (A[n].compareTo(A[maxChild]) < 0)
				swap(n, maxChild);
		}
		else {
			if (left(n) < size && A[n].compareTo(A[left(n)]) < 0)
				swap(n, left(n));
			return;
		}
